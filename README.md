...
已支持 **Windows** / **Linux** / **macOS**。Windows 可直接原生运行（无需 WSL）。
...
- **多格式输入输出**：UTF‑8、HEX、ASCII、AUTO
- **输入预处理**：去除 0x/\x/分隔符，自动补齐奇数长度
...
- **IDA 伪代码算法识别**：基于规则的快速识别
- **Hashcat / John**：按钮化参数、模板与命令预览
- **已知明文助手**：文件头/已知明文 XOR 推断
- **自定义工具**：导入命令并一键运行
...
```powershell
# Windows (PowerShell)
Get-NetTCPConnection -LocalPort 3335 | Select-Object -ExpandProperty OwningProcess | ForEach-Object { Stop-Process -Id $_ -Force }
```
...
```powershell
# Windows (如果编译依赖失败，建议安装 VS Build Tools)
...
- 工具箱：左侧菜单 → 工具箱 → 选择具体工具卡片
- Hashcat/John 路径：设置 → 工具路径
- Logo 自定义：设置 → Logo 设置
